...
当前转换流程基于官方 CodeBuddy IDE CN **4.10.1**（构建号 `33158423-3ad58bcb-cn`）验证通过。更高版本的 .deb 可能因为上游代码结构变化导致流程失败。如遇到构建失败或运行异常，请在本仓库提 Issue 并附上所使用的 .deb 版本号。
...
2. 使用 `ar`/`tar` 提取 .deb 内容，定位 CodeBuddy 应用目录；
...
**4.10.1 构建号 33158423-3ad58bcb-cn 发行说明**
4.10.1 版本发布 (2026-07-15)

本版本基于 4.10.0，包含以下修复和改进（来自上游）：

- 修复 conda 环境下 npm 变量冲突导致构建失败的问题
- 新增 CI/CD 安装测试工作流
- 改进 .deb 提取流程（使用 ar/tar 替代 dpkg-deb，无需安装 dpkg 即可解包）
- 优化原生模块重建逻辑，已有编译产物的模块不再重复编译
- 支持从 .deb 中提取桌面入口文件和 URL handler
- ImageMagick 图标缩放与多级 fallback 支持
